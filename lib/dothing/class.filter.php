...
class DOFilter extends DOBase
{
	...
	function process($source)
	{
		 ...
		 */
		//echo "<pre>71";var_export($source);exit;
		if (is_array($source))
		{
			if(!get_magic_quotes_gpc())
			{
				//$source = array_map("addslashes",$source);
				//echo "<pre>";var_export($source);exit;
			}
			//$source = $this->quoteSmart($source);
	...
	function decode($source)
	{
		...
		//var_dump($enctype);var_dump($_SERVER['CONTENT_TYPE']);var_dump($enctype && $enctype[0] == "multipart/form-data");exit;
